  ...
  const payload = buildStatusPayload({
    ...
    groups: [
      { id: 1, name: 'API', monitors: [
        { id: 10, name: 'a', daily: Array.from({ length: 90 }, () => ({ day: '', up: 100, flaky: 0, down: 0 })), slots: [], currentStatus: 0, intervalS: 60 },
        { id: 11, name: 'b', daily: Array.from({ length: 90 }, (_, i) => i === 89 ? { day: '', up: 0, flaky: 0, down: 100 } : { day: '', up: 100, flaky: 0, down: 0 }), slots: [], currentStatus: 2, intervalS: 60 },
      ]},
...
test('buildStatusPayload 24h uses slots with nodata gaps', () => {
  ...
  const intervalS = 60
  const slots = [{ startedAt: nowSec - intervalS, status: 0, intervalS }]  // 只有 1 个 slot
  const payload = buildStatusPayload({
...
test('buildStatusPayload 24h uptime counts flaky once: (up+flaky)/(up+flaky+down)', () => {
  ...
  const intervalS = 60
  const slots = [
    { startedAt: nowSec - intervalS * 3, status: 1, intervalS },   // flaky
    { startedAt: nowSec - intervalS * 2, status: 2, intervalS },   // down
  ]
  const payload = buildStatusPayload({
    siteTitle: 'S', timezone: 'UTC', range: '24h', nowSec,
...
test('overall is degraded when flaky present but no down', () => {
  const payload = buildStatusPayload({
    ...
    groups: [{ id: 1, name: 'g', monitors: [
      { id: 1, name: 'a', daily: Array.from({ length: 90 }, () => ({ day: '', up: 100, flaky: 0, down: 0 })), slots: [], currentStatus: 1, intervalS: 60 },
    ]}],
  })
